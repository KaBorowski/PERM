...
%     pause(0.1);
    ...
    for i=1:numel(s)
        ...
        if Area.Area > minPixels
            ...
            Orientation = regionprops(Obj, 'Orientation');
            Orient(i) = Orientation.Orientation;
...
MaxCoordinates = out.MaxCoordinates(1:maxLabel);
...
% Show object classes
for k = ObjectIndex
    points = MaxCoordinates{k};
    
%     text(points(1,1), points(1,2), sprintf("O"));
%     text(points(2,1), points(2,2), sprintf("O"));
    ...
    if ObjectClass{k} == "Magnes"
        text(c(1), c(2), sprintf('%s %s\nSrednica=%.2fcm', ObjectColor{k}, ObjectClass{k},...
        ObjectSize(k)), ...
        'HorizontalAlignment', 'center', ...
        'VerticalAlignment', 'middle');
    elseif ObjectClass{k} == "Dlugopis"
        dist1 = pdist2([points(1,1), c(1)], [points(1,2), c(2)]);
        dist2 = pdist2([points(2,1), c(1)], [points(2,2), c(2)]);

        if dist1 > dist2
            text(points(1,1), points(1,2), sprintf("X"));
        else
            text(points(2,1), points(2,2), sprintf("X"));
        end
        szer = Scale*minSize(k);
        text(c(1), c(2), sprintf('%s %s\nDlugosc=%.2fcm \nSzerokosc=%.2fcm\nOrientacja=%.2f', ObjectColor{k}, ObjectClass{k},...
        ObjectSize(k), szer, Orient(k)), ...
        'HorizontalAlignment', 'center', ...
        'VerticalAlignment', 'middle');
    else 
